...
//   const [TopRatings, setTopRating] = useState([])
  const [SliceRating, setSliceRating] = useState(9);

  const handleLoad = () => setSliceRating(prev => prev + 9)
  ...
  const product = Data?.slice(Data?.length - 6)?.map(product => {
    return <Cart
        ...
        price={product?.price}
        stars={product?.stars}
        href={`/catalog/product/:id/${product?.id}`}
    />
  })

  const topProduct = Data?.slice(0, SliceRating)?.map(product => {
    return product.top ? <Cart
        key={product?.id}
        type={"product"}
        horizontal
        image={product?.image}
